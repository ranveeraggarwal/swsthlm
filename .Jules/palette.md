...
## 2025-02-12 - Reusable UX Pattern: Keyboard shortcuts for hidden global actions
**Learning:** When actions are hidden behind a toggle (like a search filter panel), adding keyboard shortcuts (e.g. `/` or `Cmd+K`) provides a much faster and more pleasant experience for power users. However, these are undiscoverable without visual hints. Furthermore, when implementing keyboard shortcuts like `/`, it is critical to ensure they don't interfere with standard typing in other input fields across the application by explicitly bailing out if the active element is an input.
**Action:** Always pair global keyboard shortcuts with visual hints like `<kbd>` tags to make them discoverable. Ensure the shortcut also expands any hidden UI components necessary to focus the target element, using `setTimeout` for reliable focus management. Always check the active element before preventing default behavior for character shortcuts.
