...
import { parse } from './staclara.mjs';
...
describe('staclara parser', () => {
  ...
  it('drops non-dance noise: quiz, jams, rock, folk', () => {
    const names = events.map((e) => e.name.toLowerCase()).join(' | ');
    expect(names).not.toMatch(/quiz/);
    expect(names).not.toMatch(/jam/); // Blues Jam, Trad-Jazz Jam
    expect(names).not.toMatch(/veres/); // Finnish Folk & Psychadelica
    expect(names).not.toMatch(/blues men/); // Blues, Soul, Rock
  });
