...
## 2026-09-19 - [Focus Visible on Standalone Icons]\n**Learning:** Reusable UX Pattern: Interactive standalone icon buttons (like dismiss buttons, clear search buttons, and menu toggles) must explicitly include focus visibility styles for keyboard accessibility using Tailwind classes `focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus:outline-none` because they often lack default borders or hover states that naturally imply focus.\n**Action:** Always ensure that interactive elements without clear default outlines or boundaries have explicitly defined `focus-visible` utility classes applied.
## 2026-09-25 - [Redundant Focus Visibility Utility Classes]\n**Learning:** The project relies on a global, unlayered CSS rule in `src/app/globals.css` (`*:focus-visible { outline: 2px solid var(--primary); outline-offset: 2px; }`) that applies focus indicators universally. This rule beats Tailwind's layered `focus:outline-none` utilities. Attempting to apply `focus-visible:ring-2` utility classes to elements without default borders (like standalone icon buttons) is redundant and will result in drawing a second, duplicate ring over the global one.\n**Action:** Rely on the global `*:focus-visible` rule for standard keyboard focus indicators. Do not apply explicit Tailwind `focus-visible` utility classes unless there is a specific need to override the global outline behavior with a tactile or custom style (e.g., the `lift-card` or `lift-btn-primary` components).
