...
async function main() {
  ...
  // --- validation gate (re-parse the exact bytes we'd write) ---
  ...
  const report = buildReport({ added, updated, skipped, pastFiltered, sourceNotes, errors });
  writeFileSync(REPORT_PATH, report);
  process.stdout.write(report);

  if (errors.length) {
    console.error(`\n✖ scrape produced ${errors.length} schema error(s) — not writing.`);
    process.exit(1);
  }
...
function buildReport({ added, updated, skipped, pastFiltered, sourceNotes, errors }) {
  ...
  if (errors.length) {
    lines.push('', '### ❌ Schema errors (not written)');
