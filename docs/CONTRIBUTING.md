...
- If the PR ships a **major, user-visible feature**, add a line to the current month in [`src/lib/changelog.ts`](../src/lib/changelog.ts) — that's what the About page's "What's new" timeline renders. Data rows, scraper runs, dependency bumps, and internal refactors don't go there.
