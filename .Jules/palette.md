...
## 2026-07-20 - Visually Hidden Warning for External Links\n**Learning:** Links that open in a new tab (`target="_blank"`) should include a visually hidden warning (e.g., `<span className="sr-only"> (opens in a new tab)</span>`) to announce the context change for screen reader users.\n**Action:** When adding or modifying links with `target="_blank"`, ensure they include visually hidden text to communicate the behavior to assistive technologies.
